<?php
    ...
    $friend_id = $_GET['choosed_user_id'];
    ...
    {
      header("location: send_message.php?choosed_user_id=$friend_id");
    }

    if(isset($_POST['block_button']))
    {
      header("location: blocked_profile.php?choosed_user_id=$friend_id");
    ...
    {
      header("location: nonfriend_profile.php?choosed_user_id=$friend_id");
...
<p> PLAYLISTS <br>
<?php
  $query5 = "SELECT P.playlist_name FROM playlist P, follows F 
            WHERE F.user_id = $friend_id AND F.playlist_id = P.playlist_id
            UNION 
            SELECT P2.platlist_name
            FROM plsylist P2
            WHERE P2.creator_id = $friend_id";
  $result5 = mysqli_query($db, $query);

  for($i = 0; $array[$i] = mysql_fetch_assoc($result); $i++) ;
  
    
  while ($row5 = mysql_fetch_array($result5, MYSQLI_ASSOC)) {
    
  ?>  
    <script type="text/javascript">
      var name_p = <?php echo $result5['playlist_name']?>;
      
    </script>
      
      <td>
    <form action="" method="post">
      <input type="submit" name=name_p value="<?php echo $result5['playlist_name']; ?>"  />
    </form>
  </td>  
  ...
  <?php
    ...
    while ($row5 = mysql_fetch_array($result5, MYSQLI_ASSOC)){
      if(isset($_POST[$result5['playlist_name'] ] ) )
    {
      header("location: view_playlist_profile.php?playlist_name = $result5['playlist_name']");
    }
...
<p> POSTS <br>
<?php
  $query = "SELECT U.username , P.post FROM posts P, User U WHERE P.reciver_id = '$friend_id' AND P.writer_id = U.user_id ORDER BY date";
  $result = mysqli_query($db, $query);
